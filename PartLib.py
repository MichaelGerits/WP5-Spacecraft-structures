...
class StructuralCylinder:
    ...
    """
    def __init__(self, R=0.28, h=1.5, t=0, E=110.3e9, SigmaY=980e6, rho=4540, Poisson=0.32, buckling_k=0, half_waves=0, mass=0):
        self.R = R
        self.h = h
        self.t = t
        self.E = E
...
class TransversePanel:
    ...
    """
    def __init__(self, R_outer=1.00, t_face=0.19805e-3, t_core=15e-3, rho_face=1611, rho_core=48.2, R_struct=0.28, holes=[]):
    ...
    def calcArea(self):
        ...
        """
        area = (3 * np.sqrt(3))/2 * self.R_outer**2 #area of a hexagon
...
class Attachment:
    ...
    def CheckPullThrough(self):
        ...
        for i in range(2):   #iterate through all fastener objects
            areabolthead = (0.01/2)**2 * math.pi - (0.005/2)**2 * math.pi    #calculate area on which compressive stress acts
            sigmay = self.zload/(areabolthead * 4)      #calculate compressive stress
            areat2 = 2 * math.pi * 0.01 * self.t   #calculate areas over which the shear stress will act
